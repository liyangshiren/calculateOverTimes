...
//        return '<div class="checkbox"><input type="checkbox" name="ids" value=""/><label for="ids"/></div>'
        ...
        },
            {
                ...
                render: function (data, type, row, meta) {
                    ...
                    if (row.status != '9') {
                        if (row.status != '4') {
                            if (row.leveloneBarcodeId) {
                                div = '<div class="btn-group">' +
                                    '</div>' + '&nbsp;&nbsp;' +
                                    '<a class="btn btn-danger " title="打印一级箱标签" href="' + baseUrl + '/printOne/'
                                    + row.leveloneBarcodeId + '">打印一级箱标签</a>' + '</div>' + '&nbsp;&nbsp;'
                            }
                        }
                        if (row.twoStatus != '1') {
                            if (row.leveltwoBarcodeId) {
                                div = div + '<a class="btn  btn-danger" title="打印二级箱标签" href="' + baseUrl + '/print/leveloTwo/'
                                    + row.leveltwoBarcodeId + '">打印二级箱标签  </a>' + '</div>' + '&nbsp;&nbsp;'
                            }
                            if (row.leveltwoBarcode) {
                                div = div + '<a class="btn btn-danger " title="打印二级箱清单" href="' + baseUrl + '/print/leveloTwoList/'
                                    + row.leveltwoBarcode + '">打印二级箱清单</a>'
                            }
                        }
                    }
                    return div;
                    ...
                    /* return ('<div class="btn-group">' +
                       '</div>' +'&nbsp;&nbsp;'+
                         ...
                         '<a class="btn  btn-danger" title="打印二级箱标签" href="' + baseUrl + '/print/leveloTwo/'
                         + row.leveltwoBarcodeId + '">打印二级箱标签  </a>' + '</div>' +'&nbsp;&nbsp;'+
                         '<a class="btn btn-danger " title="打印二级箱清单" href="' + baseUrl + '/print/leveloTwoList/'
                         + row.leveltwoBarcode + '">打印二级箱清单</a>'
            ...
            {
                data: 'leveltwoBarcode',
                sortable: false,
                name: 'leveltwoBarcode',
                'class': 'text-center',
                defaultContent: ''
            },
            ...
            {
                data: 'leverTwoCompleteDate',
                sortable: false,
                name: 'leverTwoCompleteDate',
                'class': 'text-center',
                defaultContent: '',
                render: function (data, type, full) {// 全部列值可以通过full.列名获取,一般单个列值用data
                    // PS:这里的render是有多少列就执行多少次方法
            ...
            {
                data: 'container.sealNo',
                sortable: false,
                name: 'container.sealNo',
                'class': 'text-center',
                defaultContent: ''
            },
            {
                data: 'container.ladBillNo',
                sortable: false,
                name: 'container.ladBillNo',
                'class': 'text-center',
                defaultContent: ''
            },
            {
                data: 'container.sealer',
                sortable: false,
                name: 'container.sealer',
                'class': 'text-center',
                defaultContent: ''
            },
            {
            ...
            {
                data: 'container.vehicleNumber',
                sortable: false,
                name: 'container.vehicleNumber',
                'class': 'text-center',
                defaultContent: ''
            },
            {
            ...
            {
                data: 'planPackageDate',
                sortable: false,
                name: 'planPackageDate',
                'class': 'text-center',
                defaultContent: '',
                render: function (data, type, full) {// 全部列值可以通过full.列名获取,一般单个列值用data
                    // PS:这里的render是有多少列就执行多少次方法
    ...
    $('#leverTwoCompleteDate').daterangepicker({
        format: 'YYYY-MM-DD',
        showDropdowns: true,
        showWeekNumbers: true,
        timePicker: true,
        ...
        $('#leverTwoCompleteDate').val(start.format('YYYY-MM-DD') + ' - ' + end.format('YYYY-MM-DD'));
    });
...
//导出 动态组织form提交
    $(".btn.exportPackageInfo").on('click', function (e) {
        console.log(dLocNos);
        $("#exportPackageInfo .param").remove();
        var $frm = $("#exportPackageInfo");
        if (dLocNos.length == 0) {
            alert('请勾选数据');
            return
        }
