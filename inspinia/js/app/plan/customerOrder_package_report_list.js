...
//		type : "POST",
		data : function(d) {
    		d.plantNo = $("#plantNo").val();
    		d.customerOrderNo = $("#customerOrderNo").val();
    		d.customerCode = $("#customerCode").val();
    		d.receivedOrderDate = $("#receivedOrderDate").val();
    		d.requiredConsignmentDate = $("#requiredConsignmentDate").val();
    		d.partNo = $("#partNo").val();
    		return d;
		}
	},
    ...
    {data: 'customerCountryNo',name:'customerCountry', 'class': 'text-center',defaultContent: ''},
    ...
    {data: 'productionRequestPackageQty',name:'productionRequestPackageQty', 'class': 'text-center',defaultContent: ''},
    {data: 'productionRequestPackageDate',name:'productionRequestPackageDate', 'class': 'text-center',defaultContent: ''},
    {data: 'leveltwoActionPackageQty',name:'leveltwoActionPackageQty', 'class': 'text-center',defaultContent: ''},
    {data: 'leveltwoActionPackageDate',name:'leveltwoActionPackageDate', 'class': 'text-center',defaultContent: ''}
    ]
  });
  ...
  $('#search').on("click", function (e) {
	  var searchTime = $('#receivedOrderDate').val(); // 判断时间间隔
	  	if(searchTime) {
		  	...
		  	if(searchTimeCha > 30) {
		  		bootbox.alert('客户订单包装情况单次查询日期范围最大值为30天！')
		  		return false;
		  	}
	  	}
