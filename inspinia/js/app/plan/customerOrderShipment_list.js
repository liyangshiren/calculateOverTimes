...
//    		d.status = $('.search-form').find('select[name="status"]').val();
    ...
    {data: 'shipmentDate',name:'shipmentDate', 'class': 'text-center',defaultContent: ''}
   ]
  });
  ...
  $('#search').on("click", function (e) {
	  var searchTime = $('#receivedOrderDate').val(); // 判断时间间隔
	  	if(searchTime) {
		  	...
		  	if(searchTimeCha > 30) {
		  		bootbox.alert('客户订单发运情况单次查询日期范围最大值为30天！')
		  		return false;
		  	}
	  	}
	  	var plantId = $('.search-form').find('select#plant').val();
	    ...
	    table.column('plant:name').search(plantId);
